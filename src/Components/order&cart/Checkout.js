...
const Checkout = () => {
    ...
    height: 200,
     ...
     useEffect(() => {
        axios.get(`http://localhost:9901/GetAllCoupon`)
            .then((res) => {
                setcoupons(res.data);
            }).catch((err)=> console.log(err))


          axios.get(`http://localhost:9901/Get/${productid}`)
          .then((res)=> {
            setproduct(res.data);
  ...
  const getCouponDiscount = (code) => {
    axios
      .get(`http://localhost:9901/GetCouponDiscount/${code}`)
      .then((res) => {
        return res.data;
      })
      .catch((err) => console.log(err));
  ...
  const placeOrder = (e) => {
    ...
    console.log(data);
    axios.post('http://localhost:9901/placeOrder', data)
      .then((res) => {
        navigate("/myOrders");
      })
      .catch((err) => console.log(err));
  ...
  return (
    <div>
      ...
      <Typography variant="h4" sx={{ color: "red", marginLeft: 9 }}>
        Available Coupons
      </Typography>
              ...
              >
                ...
                >
                  <Typography
                    ...
                    component="div"
                  >
                     discount {coupon.couponDiscount}% for all watches
                  </Typography>
                  <Typography
                    variant="h6"
                    sx={{ color: "black" }}
